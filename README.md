...
| [`coding-hermes-retrospective-self-improvement`](coding-hermes-retrospective-self-improvement/SKILL.md) | Improvement | 改善 Kaizen — facts first (read the artifact, never the summary), quorum on the raw details, goal → subgoals → execute → **re-evaluate against the falsifier**; never plan from memory |
