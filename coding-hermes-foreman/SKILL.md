...
- **Tick storm — cooldown < tick_timeout:** If a project's cooldown is shorter than the daemon's `--tick-timeout`, the scheduler spawns overlapping ticks that all timeout. Example: 900s cooldown, 600s timeout → 5 overlapping dispatches in 2h burning $0.83. Detection: `CooldownS < 600` in scheduler API. Fix: set cooldown ≥1200s or add scheduler-level guard. **Proven:** hermes-canopy 2026-07-24.
