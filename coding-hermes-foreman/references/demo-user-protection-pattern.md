...
This is a recurring pattern. <project> hit it 4 times in 6 days (DOCKER-002 through DOCKER-005, Jul 9–15, 2026).
...
async clear(): Promise<void> {
  ...
  try {
    ...
    await admin.query(
      ...
      ['%@demo.<project>.app', 'browser-test@test.co']
    );
    await admin.query(
...
| `LIKE` glob, not a list | A pattern like `%@demo.<project>.app` covers all demo users with one condition. Adding individual emails is fragile |
| `OR email = $2` for exceptions | Users from a different domain (e.g., `browser-test@test.co` vs `@demo.<project>.app`) need an explicit exception |
...
| Raw SQL migration | Hardcoded WHERE in migration | `DELETE FROM users WHERE email NOT LIKE '%@demo.<project>.app'` |

## Prevention

When writing the seed script (`reset-and-seed.mjs`, `seed.ts`, etc.), create protected users using a consistent email domain (e.g., `%@demo.<project>.app`). The `clear()` method's WHERE clause should reference this domain.

If multiple domains are needed (admin on `@test.co`, demos on `@demo.<project>.app`), add explicit `email != 'admin@test.co'` conditions for each — but **prefer consolidating all protected users under one domain**.
...
docker exec <postgres> psql -U <user> -d <db> \
  -c "SELECT email, role FROM users WHERE email LIKE '%@demo.<project>.app' OR email = 'browser-test@test.co'"

# 3. Verify demo login still works
curl -X POST /api/v1/auth/login \
  -H "Content-Type: application/json" \
  -d '{"email":"teacher@demo.<project>.app","password":"Edu0sDemo!}"'
# Expect: HTTP 200 with JWT
```

## Proven In

- **<project>** DOCKER-004 (Jul 15, 2026): Added `NOT LIKE '%@demo.<project>.app'` protection. 1681 tests pass, 6 demo users intact.
- **<project>** DOCKER-005 (Jul 15, 2026): Extended to also protect `browser-test@test.co` (different domain). Added `OR email = $2` to subquery. 7 demo users intact.
