...
| **Foreman** | deepseek-v4-pro | **PAYG** (deepseek-foreman) | Must always load. Always unblockable via credit. |
| **Foreman (idle)** | deepseek-v4-flash | **PAYG** (deepseek-foreman) | Same rule — presence non-negotiable. |
| **Worker (heavy)** | glm-5.2 / MiniMax-M3 / gpt-5.6 | Prepaid flat-rate | Burn quota. Foreman switches if bucket locks. |
| **Worker (light)** | grok-4.5 / kimi-k2.7 | Prepaid flat-rate | Bug fixes. Same bucket-switching logic. |
...
5. Report orphans that cannot be registered to Bane — they need manual addition to the scheduler's SQLite database. Known orphans (as of 2026-07-20): helios-work (5 pending, has go.mod, full Go codebase), hermes4friends (4 pending, no go.mod — may be umbrella or config project)
5. Use the scheduler API's `GET /api/v1/projects` response to differentiate between existing projects (which can be updated) and genuine orphans (which return 404 on GET)
...
**⚠️ Daemon running but HTTP unreachable — detection gap:** A `curl` to `:9090` may return connection refused/timeout even though `ps aux | grep schedulerd` shows a running process and ticks are still spawning in the process table. This happens when the HTTP server within the daemon is wedged (deadlock in the HTTP mux, stuck goroutine) while the timer/scheduler loop still runs. **Detection:**
...
Count open tasks via `grep -c '^## \[ \]' .coding-hermes/tasks.md`:
- **3+ pending** → 15m (900s) + coding-model (v4-pro) — active project, needs fast iteration
- **1-2 pending** → 30m (1800s) + coding-model (v4-pro) — moderate activity
- **0 pending** → 2h (7200s) + budget-model (v4-flash) — idle, slow down to save budget
- **CI-aware:** Even projects with 0 pending tasks but failing CI should stay at moderate speed (600-3600s, default 1800s) until CI is fixed — CI fix tasks count as pending work. **Proven:** 2026-07-19 — dexdat-memory and speclang had 0 pending tasks but CI failing with 3 consecutive failures each. Set to 1800s (30min) instead of 7200s (2h).
...
##### Model assignment by project stage (Phase 2C reference)

When the user specifies model assignment by project stage (as in the 2026-07-19 supervisor tick), use this taxonomy:
| Stage | Model | Provider rationale | Use case |
|-------|-------|-------------------|----------|
| **Scaffold** | kimi-for-coding | Prepaid flat-rate | New projects, prototyping, boilerplate |
| **Active Go/Python** | glm-5.2 (via zai-glm) | Prepaid flat-rate | Production coding, features, refactors |
| **Planning** | deepseek-v4-pro | PAYG (deepseek-foreman) | Architecture, design, spec writing |
| **Budget** | deepseek-v4-flash | PAYG (deepseek-foreman) | Idle maintenance, discovery sweeps |

Foremen always run on `deepseek-foreman` PAYG. The stage-based model applies to the scheduler project's Model field for the foreman session. If the user does not provide explicit stage assignments, fall back to the cooldown-based model rules (v4-pro for ≤30m, v4-flash for ≥60m).
...
|---------|------|---------|
| 2.30.0 | 2026-07-20 | **Phase 0G expanded.** Added Check 3 — enabled projects with BOTH Model and Provider empty (helios pattern). Previously only caught cases where one was set and the other empty. **Phase 0D name normalization.** Added note to normalize underscore↔hyphen when comparing scanned directory names to scheduler project names, preventing false-positive orphan flags (ai_plays_poke vs ai-plays-poke). **Phase 2D non-foreman guard.** Added exclusion rule for purpose-specific projects (E2E testers, watchdogs) that share a workdir with a foreman — the pending-task formula from the shared board is meaningless for them. Proven: eduos-e2e slowed from 900s→7200s based on foreman's 0 pending tasks. |
| 2.29.0 | 2026-07-20 | **Phase 0D orphan differentiation added.** Umbrella directories (coding-hermes-scheduler, get-h3) with inherited task boards but no source code markers were being flagged as orphans. Added source-code-marker check (go.mod/package.json/setup.py) before flagging. **Foreman self-disable triage criteria added.** When re-enabling self-disabled foremen, use triage: re-enable if pending tasks > 0 or has active codebase; leave disabled if 0 pending or empty workdir. Proven: muster (1 pending → re-enabled), musterflow (0 pending → re-enabled but no work). **Proven:** 2026-07-20 — helios-work (go.mod + 5 pending = real orphan), get-h3 (no go.mod = umbrella, skipped). |
| 2.28.0 | 2026-07-20 | **Phase 0G scheduler-model cross-check expanded.** Added Check 2 — provider consistency verification. The scheduler API's Provider field is a Go string that can be `""` (empty) while Model is set, causing spawns to inherit an unplanned default provider. Phase 0G now checks both Model alignment and Provider non-empty. Fixed eduos-e2e (`Provider=""` → `deepseek-foreman`). New reference: `references/scheduler-api-empty-provider-check.md` with detection/fix recipe. **Proven:** 2026-07-20 — eduos-e2e had Model=v4-pro but Provider=``, spawning without provider override. |
| 2.27.0 | 2026-07-20 | **New pitfalls: BlockingIOError fork contention + duplicate tick spawn.** Documented the `Errno 11` resource contention pattern when 6+ concurrent foreman ticks each spawn MCP subprocesses — detection via `journalctl`, recovery via `pkill -f` stale ticks. Documented the concurrent duplicate tick spawn (ai-plays-poke had 2 concurrent ticks). **Phase 0I expanded:** added `--noproxy '*'` flag requirement for curl inside gateway container, added `ss -tlnp` and `journalctl` fallback detection when HTTP port is unreachable but daemon is still producing ticks. **Proven:** 2026-07-20 — daemon HTTP unreachable for entire supervisor tick, background process producing ticks, curl inside gateway required `--noproxy '*'` bypass. New reference: `references/blockingioerror-fork-contention.md`. |
| 2.25.0 | 2026-07-19 | **Phase 0E expanded.** Added scheduler case-sensitivity duplicate detection (`heading`/`HEADING`). Added "not all shared-workdir projects are duplicates" (eduos-e2e + eduos.dexdat.com.co). **AutoSlowdown cooldown reversion pitfall** — projects with LastTickAt=never have API-set cooldowns reverted by daemon autoSlowdown within 60s. New reference: `references/autoSlowdown-cooldown-reversion-never-fired.md`. **CI-aware rule proven:** dexdat-memory and speclang set to moderate speed (1800s) for 0-pending + CI-failing. **Empty workdir foreman pitfall:** eduos foreman targeting empty workdir burns PAYG on discovery sweeps. |
| 2.23.0 | 2026-07-18 | **Phase 2D API reference fixed.** The individual GET endpoint `GET /api/v1/projects/<name>` returns a nested structure `{"latest_tick":..., "project":{...}}` — not a flat JSON. The old API reference example omitted the nesting, causing verification scripts to read `d.get('Model')` (returns `None`) instead of `d['project']['Model']`. Updated with distinct list-endpoint vs individual-endpoint examples and a pitfall. **New pitfalls:** scheduler API route case sensitivity (`heading` vs `HEADING`), sub-project task board mapping for umbrella repos (h3 SDKs under `get-h3/`). |
| 2.21.0 | 2026-07-18 | **New pitfall: MCP memory duplication.** Each hermes chat spawns per-instance MCP servers (DuckBrain 103MB, Flights 76MB, GitReins 33MB, Chrome 350MB). 8 concurrent = 4GB duplicated. Chrome disabled via `disabled_toolsets: [browser]` (350MB saved per chat, 60% reduction). New reference: `references/mcp-memory-duplication.md` with memory breakdown, detection commands, and fix path (switch MCPs from stdio to HTTP daemon mode). **Proven:** 2026-07-18 — scheduler daemon heap 70KB, all 26.8GB peak from subprocess MCP duplication. |
| 2.20.0 | 2026-07-18 | **Phase 0B: NEVER re-enable paused foreman crons.** The old fallback "when daemon is inactive → re-enable paused cron foremen" created a destructive cycle: daemon dies → supervisor re-enables old crons → duplicate ticks burn PAYG → daemon restarts → conflict. Now: when daemon is inactive, restart it (Phase 0I). NEVER touch foreman cron state. **New Phase 0I: Scheduler Daemon Health** — proactively checks daemon liveness, restarts with correct `-db` flag, alerts Bane after 3 failed attempts. **Phase 0D:** Removed "When daemon is INACTIVE: Auto-create a foreman cron" — cron foremen are DEPRECATED. **DB path documented:** Binary defaults to `~/.hermes/scheduler.db` but real data is at `~/.hermes/coding-hermes/scheduler.db`. Symlink created. **Proven:** 2026-07-18 — daemon died at 05:35 with "HTTP shutdown: context deadline exceeded", supervisor tick at 08:00 would have re-enabled all 27+ paused foreman crons. |
| 2.16.0 | 2026-07-18 | **Phase 6 CI Check: `.env` sourcing pitfall** — `source ~/.hermes/.env` fails from cron scripts (non-bash-safe content). Added grep-based GITHUB_PAT extraction recipe (bash + Python). **Proven:** 2026-07-18 — supervisor errored on `source ~/.hermes/.env`. |
| 2.15.0 | 2026-07-17 | **Phase 0H pitfall: cron-interval parser false positives on daily/weekly DuckBrain sync crons.** The `parse_cron_interval()` function defaults to 3600s for fixed-value cron expressions (`0 3 * * *` → 1h not 24h), causing 47 false-positive stale flags. New reference: `references/phase0h-cron-interval-parsing-gap.md` with `compute_cron_interval()` — a robust parser that handles daily, weekly, and monthly cron expressions. SKILL.md Phase 0H updated with detection and fix guidance. **Proven:** 2026-07-17 — 47 DuckBrain sync crons incorrectly flagged; 37 on daily schedules, perfectly healthy. |
| 2.14.0 | 2026-07-17 | **Phase 0G expanded — script quality verification.** The supervisor previously only checked that the enforcement cron was `scheduled`+`enabled` — not that the script itself contained the right rules. Added Script quality verification step after the enforcement-cron health check: scans for Rule 0 (toolset normalization), Rule 1 (provider), Rule 2 (model), and the foreman-detection guard. **Proven 2026-07-17** — `enforce-foreman-models.py` ran every 30m for 3+ days with zero toolset enforcement because the v2.4.0 changelog entry described the intended fix but the actual script was never updated. **Pitfall updated:** replaced `set(new_et) != set(et)` text with the real story — code was missing entirely, not just broken. The 2.12.0 changelog entry described intent; this entry records the deployment. |
| 2.13.0 | 2026-07-17 | **Phase 0B scheduler daemon guard.** Added daemon check before unpausing paused-without-reason foremen — when the scheduler daemon is active on :9090, foreman state is the daemon's responsibility. Script `phase0-autoheal.py` now skips the `paused → scheduled` rule for foremen when daemon is active. **False positive:** 38 foremen were unpaused in one tick because paused_reason wasn't set during migration. New reference: `references/scheduler-migration-paused-recovery.md` with recovery script and active-foreman restore edge case. SKILL.md Phase 0B updated with guard instructions. |
| 2.12.0 | 2026-07-17 | **Designed: toolset normalization for `enforce-foreman-models.py`.** Intended to add null→canonical, strip delegation/cronjob, add missing tools. **This was NOT deployed to the actual script on disk** — the changelog entry described the intended fix but the file at `~/.hermes/scripts/enforce-foreman-models.py` was never updated. The script ran for 3+ days with zero toolset enforcement. **Deployed in 2.14.0** (2026-07-17 supervisor tick). Pitfall updated to reflect the real gap (missing code, not broken code). Phase 0G expanded with script quality verification to prevent recurrence. |
| 2.10.1 | 2026-07-15 | **New Phase 0C pattern: silent scheduler skip.** Added detection and recovery for foremen where `next_run_at` advances every tick but the agent session never starts — no fire_claim, no error, no last_status. This is distinct from fire_claim-blocked jobs and `kind: "once"` self-destructs. Added `references/silent-scheduler-skip-pattern.md` with detection script and recovery attempts. **Proven:** 2026-07-15 — hermes-dagger-coding-foreman created at 18:19 CT, next_run_at advanced through 3+ ticks, never fired. |
| 2.8.0 | 2026-07-15 | **Phase 0H expanded:** Added "never ran" subcase for infra cron stale detection (`last_run_at: null` + age > 24h). Distinguishes fire_claim-blocked (different recovery) from scheduler-never-dispatched (force-fire). **Proven:** 2026-07-15 — `crier-duckbrain-sync` had never run since creation, no fire_claim, fixed instantly. |
| 2.7.0 | 2026-07-14 | **New Phase 0H: Infra Cron Stale Recovery** — DuckBrain sync crons and other infra crons can silently freeze after Hermes updates. Added detection pattern (check `last_run_at` age > 3× interval), recovery (batch force-fire), and cluster detection logic. Added `references/infra-cron-stale-recovery.md` with full Python script template. **Proven:** 2026-07-14 — 15 DuckBrain sync crons frozen since 2026-07-08 (7 days), all recovered via batch force-fire. Also added **Phase 0G: Foreman Model Drift** as an explicit Phase 0 step (was only documented inline in scripts). |
| 2.5.0 | 2026-07-14 | **New pitfall: batch foreman creation leaks model/provider onto non-foreman crons.** When creating foremen via `hermes cron create` + JSON patch, jobs with `coding-hermes` (not `coding-hermes-foreman`) in skills inherit model/provider/toolsets/delegation from the batch. Post-heal verify catches it, but healing scripts correctly skip non-foremen. New reference: `references/crier-foreman-leak-pattern.md`. |
| 2.4.0 | 2026-07-14 | **Critical fix: `phase0-autoheal.py` 0G model check reversion bug.** Old check `model != "deepseek-v4-pro"` forced ALL foremen to v4-pro, reverting idle foremen correctly set to v4-flash by Phase 2D. Fixed to allow both v4-pro and v4-flash (matching `enforce-foreman-models.py`). **Toolset enforcement added to `enforce-foreman-models.py`:** Now auto-fixes null toolsets, strips delegation/cronjob, adds missing search/skills/memory — eliminating the 32-issue post-heal failure pattern. New pitfall documented. |
| 2.3.0 | 2026-07-14 | **CI check flow:** Fixed `gh run view` pitfalls — `--limit` not valid for view, `-R` requires `OWNER/REPO` not directory path. **Phase 0E edge case:** When both duplicate foremen have `coding-hermes-cron`, differentiate by `coding-hermes-foreman` presence instead. **New reference:** Added `references/2026-07-14-supervisor-tick.md` — documents the Jul 13 mass 402 provider recovery, crier duplicate fix, and `hermes cron edit` success for cron→interval conversion. |
| 2.2.0 | 2026-07-14 | **New reference:** Added `references/ci-gh-log-access-limitations.md` — documents the `gh run view --log` HTTP 404 pattern when the token lacks log-read permission for a repo's owner org. Added pitfall about non-guaranteed CI log access. Phase 6 CI Check updated with detection flow and three-case classification. Security scanner workarounds expanded to cover pipe-to-interpreter patterns (`gh | python3`). |
| 2.1.2 | 2026-07-13 | **New reference:** Added `references/provider-credit-exhaustion-mass-outage.md` — documents the fleet-wide HTTP 402 pattern when the shared `deepseek-foreman` PAYG provider runs out of credit (25 foremen crashed simultaneously 2026-07-13). Added pitfall: detect temporal clusters of foreman errors on same provider as provider-level outage, not individual failures. | — `re.search(r'\\*/(\\d+)', '0 */2 * * *')` matched `*/2` in the HOUR field, returning 2m instead of 2h, causing false-positive force-fires on 10 cron-kind foremen. Added `parse_cron_interval()` that checks minute field first, then hour field. Added `scripts/post-heal-verify.py` — formalized verification script. |
| 2.1.0 | 2026-07-13 | **Critical fix:** Removed stale pinned table from `scripts/phase0-autoheal.py` that reverted 4 foremen (bunker, speclang-ci, mythos, helix) back to prepaid providers on every tick. Added post-heal verification step to script execution order (Phase 0 prelude). |
| 2.0.0 | 2026-07-12 | Full rewrite with Phase 0 auto-heal, gateway shutdown recovery, CI checks, HTML report generation. |

## Pitfalls (Fleet-Specific)
...
- **autoSlowdown reverts API-set cooldowns on never-fired projects.** When a scheduler project has `LastTickAt=never/null` (never completed a tick), the daemon's `autoSlowdown` function detects "IDLE" and multiplies cooldown by 1.5× every evaluation cycle (~60s). Any API-set cooldown is overwritten within ~60-120s. **Detection:** set cooldown, re-query 60s later — if reverted, check `d['project']['LastTickAt']` (or `last_tick_at`). **Fix:** The project needs one successful tick before the cooldown stabilizes. Until then, accept the autoSlowdown and reset cooldown on every supervisor tick. Bumping Priority/Weight may help the project get dequeued and fire. See `references/autoSlowdown-cooldown-reversion-never-fired.md`. **Proven:** 2026-07-19 — asce (6 pending tasks) set to CooldownS=900 three separate times; each was reverted to 14400s within 60s because LastTickAt=never.
