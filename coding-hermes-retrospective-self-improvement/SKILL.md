...
description: "Use when asked to reflect, run a retrospective, plan it / outline it / make subgoals, verify, ask 'is it actually done', or stop mid-task to check whether the thing works — facts-first goal to subgoals, quorum on the raw details, then re-evaluate against the falsifier. Never plan from memory."
version: 1.0.0
...
- **Canonical skill name:** `coding-hermes-retrospective-self-improvement` (correct spelling:
  *self-improvement*, hyphenated). The earlier spelling `coding-hermes-retrospective-selfimprovement`
  and the display name **改善 Kaizen** both refer to THIS skill — do not create a second one.
- **Before creating or rebuilding any skill, plan, or row: search for the existing one.** The
  board (`tasks.jsonl`), the skills repo, and the session history all get searched FIRST. Both
  the cost-per-task engine and this very skill were once re-derived as new work while they
  already existed; that is the same failure class as planning from memory, one level up.
...
- **A capability marked complete because the CODE exists.** (Measured: a "COST-PER-TASK ENGINE"
  and a "STATS ENGINE V2" both sat `complete` on the board while the ordering they exist to
  drive was still `price`, and the switch that would enable it was reachable by nobody.) A
  done-when must name the EFFECT on live data — a file, a flag, or a unit test passing is not it.
- **A rate computed over a contaminated sample.** Own probes, one-off experiments and imported
  ...
  n=1 is not evidence, it is an anecdote.
- **Re-deriving a standing requirement as a new discovery.** Search the board and the history
  first; the requirement may already be written, already built, or already FAILED with the
  reason attached (a failed row naming the exact blocker is the most valuable thing on the board).
- **Quorum-by-deliberation-MCP instead of quorum-by-seats.** A multi-model deliberation is not a
  quorum: the quorum is independent judge SEATS on full sessions, each working the SAME claim
  checklist, with the coordinator re-verifying every CONTRADICTED lead as the commit layer.
